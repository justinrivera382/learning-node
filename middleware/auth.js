...
exports.protect = asyncHandler(async (req, res, next) => {
  ...
  if (
    ...
    // set token from Bearer token in header (one of two options), gotta pick one though
  ...
  // set token from cookie (one of two options), gotta pick one though
  // cookies allow us to maintain a certain length of "memory" of the current session allowing the user to continuously login without having to use the "bearer" token. it's convenient
  // this is just a convenient option to log a user in
